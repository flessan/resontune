 ...
 */
export type KickKind = 'perfect' | 'great' | 'good' | 'miss' | 'clear' | 'drop' | 'chord' | 'fever' | 'hold';
...
export function playKick(
  ...
  const pitch = kind === 'perfect' || kind === 'fever' ? 1.08 : kind === 'great' ? 1 : kind === 'chord' ? 0.92 : kind === 'hold' ? 0.8 : 0.86;
  const body = kind === 'fever' ? 1.05 : kind === 'perfect' ? 1 : kind === 'clear' ? 0.85 : kind === 'hold' ? 0.82 : 0.7;
  const click = kind === 'perfect' || kind === 'fever' || kind === 'chord' ? 1 : kind === 'hold' ? 0.4 : 0.5;
  ...
  bodyGain.gain.exponentialRampToValueAtTime(0.12 * body, when + 0.006);
  ...
  clickGain.gain.exponentialRampToValueAtTime(0.038 * click, when + 0.002);
  ...
  if (kind === 'perfect' || kind === 'fever') {
    ...
    subGain.gain.exponentialRampToValueAtTime(0.05, when + 0.008);
