...
import { DIFFICULTIES, HIT_GOOD, HIT_OK, HIT_PERFECT } from './types';

const N = DIFFICULTIES.normal;

describe('timing windows (FLOW 0.3 Normal)', () => {
  it('awards PERFECT / GOOD / OK inside the original windows', () => {
    ...
    expect(judgeTiming(N.ok, N).kind).toBe('ok');
    expect(judgeTiming(N.good, N).points).toBe(HIT_GOOD);
    expect(judgeTiming(N.ok, N).points).toBe(HIT_OK);
  });

  it('rejects too-early taps without consuming the note', () => {
    const j = judgeTiming(-N.ok - 0.001, N);
...
describe('accuracy and grades', () => {
  ...
  it('restores a little health on a hit', () => {
    ...
    expect(healthOnHit('good', 99)).toBe(100);
    expect(healthOnHit('ok', 50)).toBe(51);
  });
});
