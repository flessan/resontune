...
export default function Discover() {
  const { data: genres } = useFetch<{ genres: GenreInfo[] }>('/genres');
  const { data: tags } = useFetch<{ tags: TagInfo[] }>('/tags');
  const { data: albums } = useFetch<{ albums: Album[] }>('/albums?limit=12');
  const { data: collections } = useFetch<{ collections: Collection[] }>('/collections');
  const { data: latest } = useFetch<{ tracks: Track[] }>('/tracks?limit=10');
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 26 }}>
        {(genres?.genres ?? []).filter((g) => g.trackCount > 0).map((g) => (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 26 }}>
        {(tags?.tags ?? []).filter((t) => t.trackCount > 0).map((t) => (
